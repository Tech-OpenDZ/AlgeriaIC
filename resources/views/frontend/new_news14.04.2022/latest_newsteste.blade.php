
@if($latest_news->isEmpty())
<div class="row mt-3 mb-3">
    <div class="col-lg-12 col-md-12 col-sm-12 col-12">
        <p class="news-post-caption">@lang('news.noNewsFound')</p>
    </div>
</div>
@else
<div class="carousel-inner">
    @for($i = 0; $i < count($latest_news);)
    <div class="carousel-item @if($i==0) active @endif">
        <div class="row mb-3">
           
            <div class="col-xl-6 col-lg-12 col-md-12 mt-3">
                <div class="news-post-outer news-post-height">
                    <div class="news-post__left">
                        <div class="ratio-16x9">
                            <div class="ratio-inner">
                                <a href="{{route('news-detail', [$latest_news[$i]->page_key])}}">
                                    <img src="{{asset('storage/uploads/news_logos/'.$latest_news[$i]->news_logo)}}" alt="eco-news" class="img-fluid eco-news-img">
                                </a>
                            </div>
                        </div>
                    </div>
                    <div class="news-post__right">
                        <ul class="tags-top">
                            @foreach($latest_news[$i]->sectors as $sector)
                            @break($loop->iteration == 4)
                            <li> <a href="{{url()->current().'?page=1&keyword=&sector%5B%5D='.$sector->id}}" class="yellow-box">{{$sector->localeAll[0]->name}}</a></li>
                            @endforeach
                        </ul>
                        <p class="news-time">{{Carbon\Carbon::parse($latest_news[$i]->created_at)->diffForHumans()}}</p>
                        @if($latest_news[$i]->is_premium == 1)
                        <a href="{{route('premium-news-detail', [$latest_news[$i]->page_key])}}">
                            <h6 class="news-text-heading text-limit-news">{{ html_entity_decode(strip_tags($latest_news[$i]->localeAll[0]->title)) }}</h6>
                           
                            <span>
                                <a href="#" class="premium-eco">@lang('news.premiumNews')</a>
                            </span>    
                        </a>
                        @else
                        <a href="{{route('news-detail', [$latest_news[$i]->page_key])}}">
                            <h6 class="news-text-heading text-limit-news">{{ html_entity_decode(strip_tags($latest_news[$i]->localeAll[0]->title)) }}</h6>    
                        </a>
                        @endif
                        <a href="{{$latest_news[$i]->source_link}}" target="_blank"><p class="source-news"><img src="{{asset('storage/uploads/news_source/logo/'.$latest_news[$i]->newsSource->logo)}}"  alt="algeria-logo" class="" height="24px" width="60px"></p></a>
                    </div>
                </div>
            </div>
            @php
                $i++;
            @endphp
            <div class="col-xl-6 col-lg-12 col-md-12">
                @if($i < count($latest_news))
                <div class="news-post-outer mt-3">
                    <div class="row">
                        <div class="col-xl-5 col-lg-2 col-md-3 col-sm-2 col-12 no-padding-right">
                            <div class="news-post__left news-small-box">
                                <div class="ratio-1x1">
                                    <div class="ratio-inner">
                                        <a href="{{route('news-detail', [$latest_news[$i]->page_key])}}">
                                            <img src="{{asset('storage/uploads/news_logos/'.$latest_news[$i]->news_logo)}}" alt="eco-news" class="img-fluid eco-news-img">
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-7 col-lg-10 col-md-9 col-sm-10 col-12">
                            <div class="news-post__right">
                                <ul class="tags-top">
                                    @foreach($latest_news[$i]->sectors as $sector)
                                    <li> <a href="{{url()->current().'?page=1&keyword=&sector%5B%5D='.$sector->id}}" class="yellow-box">{{$sector->localeAll[0]->name}}</a></li>
                                    @break($loop->iteration ==2)
                                    @endforeach
                                </ul>
                                <p class="news-time">{{Carbon\Carbon::parse($latest_news[$i]->created_at)->diffForHumans()}}</p>
                                <a href="{{route('news-detail', [$latest_news[$i]->page_key])}}">
                                    <h6 class="news-text-heading-two text-limit-three">{{ html_entity_decode(strip_tags($latest_news[$i]->localeAll[0]->title)) }}</h6>
                                    @if($latest_news[$i]->is_premium == 1)
                                    <span>
                                        <a href="#" class="premium-eco">@lang('news.premiumNews')</a>
                                    </span>
                                    @endif
                                </a>
                                <a href="{{$latest_news[$i]->source_link}}" target="_blank"><p class="source-news small-box-source"><img src="{{asset('storage/uploads/news_source/logo/'.$latest_news[$i]->newsSource->logo)}}"  alt="algeria-logo" class="" height="24px" width="60px"></p></a>
                            </div>
                           
                        </div>
                    </div>
                </div>
                @endif
                @php
                    $i++;
                @endphp
                @if($i < count($latest_news))
                <div class="news-post-outer mt-3">
                    <div class="row">
                        <div class="col-xl-5 col-lg-2 col-md-3 col-sm-2 col-12 no-padding-right">
                            <div class="news-post__left news-small-box">
                                <div class="ratio-1x1">
                                    <div class="ratio-inner">
                                        <a href="{{route('news-detail', [$latest_news[$i]->page_key])}}">
                                            <img src="{{asset('storage/uploads/news_logos/'.$latest_news[$i]->news_logo)}}" alt="eco-news" class="img-fluid eco-news-img">
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-7 col-lg-10 col-md-9 col-sm-10 col-12">
                            <div class="news-post__right">
                                <ul class="tags-top">
                                    @foreach($latest_news[$i]->sectors as $sector)
                                    <li> <a href="{{url()->current().'?page=1&keyword=&sector%5B%5D='.$sector->id}}" class="yellow-box">{{$sector->localeAll[0]->name}}</a></li>
                                    @break($loop->iteration ==2)
                                    @endforeach
                                </ul>
                                <p class="news-time">{{Carbon\Carbon::parse($latest_news[$i]->created_at)->diffForHumans()}}</p>
                                <a href="{{route('news-detail', [$latest_news[$i]->page_key])}}">
                                    <h6 class="news-text-heading-two text-limit-three">{{ html_entity_decode(strip_tags($latest_news[$i]->localeAll[0]->title)) }}</h6>
                                    @if($latest_news[$i]->is_premium == 1)
                                    <span>
                                        <a href="#" class="premium-eco">@lang('news.premiumNews')</a>
                                    </span>
                                    @endif
                                </a>
                                <a href="{{$latest_news[$i]->source_link}}" target="_blank"><p class="source-news small-box-source"><img src="{{asset('storage/uploads/news_source/logo/'.$latest_news[$i]->newsSource->logo)}}"  alt="algeria-logo" class="" height="24px" width="60px"></p></a>
                            </div>
                           
                        </div>
                    </div>
                </div>
                @endif
                @php
                    $i++;
                @endphp      
            </div>
            
        </div>
    </div>

    @endfor
</div>
@endif
