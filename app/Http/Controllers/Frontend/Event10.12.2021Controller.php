...
class EventController extends Controller
{
    public function index(Request $request)
    {
        ...
        */
        ...
        if ($request->has('start_date') && !empty($request->get('start_date'))) {
            $upcomingEvents = $upcomingEvents->whereDate('start_date', '>', Carbon::now())
            ->whereDate('start_date', '>=', Carbon::parse($request->get('start_date')));
        }
        else {
            $upcomingEvents = $upcomingEvents->whereDate('start_date', '>=', Carbon::now());
        }
        ...
        */
        ...
        if ($request->has('start_date') && !empty($request->get('start_date'))) {
            $pastEvents = $pastEvents->whereDate('start_date', '<', Carbon::now())
            ->whereDate('start_date', '<', Carbon::parse($request->get('start_date')));
        }
        else {
            $pastEvents = $pastEvents->whereDate('start_date', '<', Carbon::now());
    ...
    public function listPastEvent(Request $request)
    {
        ...
        if ($request->get('action') == 'search-by-dates') {
            ...
            if ( $request->has('sort_by') && $request->get('sort_by') == 'Week') {
                ...
                $pastEvents = $pastEvents->whereDate('start_date', '<', $start_date)
        ...
        $pastEvents = $pastEvents->simplePaginate(8);
    ...
    public function listUpcomingEvent(Request $request)
    {
        ...
        $upcomingEvents = $upcomingEvents->simplePaginate(8);
