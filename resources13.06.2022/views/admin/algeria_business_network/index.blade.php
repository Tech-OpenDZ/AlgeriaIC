...
@php
    if (\Auth::user()->can('algeria-business-network-edit') || \Auth::user()->can('algeria-business-network-delete')) {
        $actionEnable = true;
    }
    else {
        $actionEnable = false;
...
<div class="card card-custom">
	<div class="card-header">
		<div class="card-title">
			<h3 class="card-label">Manage Algeria Business Network</h3>
		</div>
		@can('algeria-business-network-create')
		<div class="card-toolbar">
			<a href="{{route('manage-algeria-business-network.create')}}" class="btn btn-primary">
				<span class="svg-icon svg-icon-md">
				</span>New Algeria Business Network
			</a>
		</div>
		@endcan
	</div>
...
$(function () {
    var table = $('#partner_datatable').DataTable({
        ...
        serverSide: true,
        ajax: {
          url: "{{ route('manage-algeria-business-network.index') }}",
          data: function (d){
