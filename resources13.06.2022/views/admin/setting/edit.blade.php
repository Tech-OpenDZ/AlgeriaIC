        ...
        <div class="card-header">
            <div class="card-title" style="width: 100%;">
                ...
                <div class="pull-right">
                    <a class="btn btn-primary" href="{{ route('manage-setting.index') }}">Back</a>
                </div>
            </div>
        </div>
        <!--begin::Form-->
        <div class="card-body">
            {!! Form::open(['method' => 'post', 'route' => isset($setting->id) ? ['manage-setting.update', $setting->id] : ['manage-setting.store'], 'id' => 'eit_setting_form']) !!}
                <div class="card-body">
                    @csrf
                    @if(isset($setting->id))
                        @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        {!! Form::checkbox('status',1, isset( $setting->status) ? $setting->status: 1) !!}
                        {!! Form::label('status', 'Active/Inactive',['class' => 'required-class','data-parsley-required' => 'true']) !!}
                    </div>
                </div>
                <div class="card-footer">
                    <button type="submit" class="btn btn-primary mr-2">Submit</button>
                    <a class="btn btn-secondary" href="{{ route('manage-setting.index') }}">Cancel</a>
                </div>
            {!! Form::close() !!}
        </div>
    </div>
