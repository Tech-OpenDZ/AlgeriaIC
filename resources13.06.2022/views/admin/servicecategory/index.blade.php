...
@php
    if (\Auth::user()->can('service-category-edit') || \Auth::user()->can('service-category-delete')) {
        $actionEnable = true;
    }
    else {
        $actionEnable = false;
    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            <div class="card-title">
                <h3 class="card-label">Manage Service Category
            </div>
            <div class="card-toolbar">
                @can('service-category-create')
                <!--begin::Button-->
                <a href="{{route('manage-service-category.create')}}" class="btn btn-primary">
...
@section('sctipts')
     ...
     <!-- ServiceCategory Delete confirmation model -->
    <div id="deleteServiceCategoryModal" class="modal fade delete_admin_btn" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <form name='deleteServiceCategoryForm' id='deleteServiceCategoryForm'  method="POST">
                    {{method_field('DELETE')}}
                    {{ csrf_field() }}
    ...
    <script>
        $(function () {
            ...
            var table = $('#data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('manage-service-category.index') }}",
                    data: function (d) {
        ...
        $(document).on('click', ".delete_admin_btn", function(e) {
            ...
            $('#deleteServiceCategoryForm').attr('action', $(this).data('href'));
            $('#delete_hidden').val(del_id);
            $('#deleteServiceCategoryModal').modal('show');
        });
