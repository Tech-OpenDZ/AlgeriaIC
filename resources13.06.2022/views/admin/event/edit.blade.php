...
<div class="card card-custom gutter-b" style="width: 100%;margin: 0 auto;">
    ...
    <div class="card-body">
    {!! Form::open(array('method'=>'POST','route' => ['manage-event.update',$event->id],'files'=>true, 'data-parsley-validate')) !!}
    {{ method_field('PATCH') }}
...
<script>
    $(document).ready(function() {
        ...
        $(".removeSelectedReference").click(function(){
            var data = $(this).data('id');
            $('#delete_reference_hidden').val(data);
            $('#removeReferenceModal').modal('show');
        });
        $(".remove-exhibitor").click(function(){
            var data = $(this).data('id');
            $('#delete_exhibitor_hidden').val(data);
            $('#removeExhibitorModal').modal('show');
        });
        ...
        $('body').on('click', ".delete-document", function(e) {
            ...
            console.log(del_id);
            $('#deleteDocumentForm').attr('action', $(this).data('href'));
            $('#delete_document_hidden').val(del_id);
